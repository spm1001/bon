...
**Dolt write strategy is item-grain diff-at-save** (bon-resena, 2026-08-23 — this REVERSES the earlier "truncate-and-reinsert, never per-item" doctrine, with cause). `dolt_load_items` snapshots the normalised board per prefix; `dolt_save_items` diffs against it and writes only changed/new rows plus explicit deletions, so concurrent lanes editing different items commute. The old whole-prefix rewrite let a load that caught another lane's write mid-flight persist a half-board as truth — 7 rows lost in production under two ordinary lanes (restored from Dolt history), 42 of 60 in the scripted reproduction. Loads read `AS OF 'HEAD'` (committed state — Dolt's sql-server otherwise serves other connections' in-flight transaction state to plain reads). The population fallback (a save with no prior load in-process: migrate, init import) still rewrites the whole prefix, deliberately — those paths are single-lane. JSONL keeps whole-file rewrite: its concurrency story is atomic rename plus git merge, which was never actually parallel to Dolt's.
...
**Notes room:** `~/notes/practices/cohabiting-codebases/` — the pattern language this tool is the largest worked example of. (bon-jezahi, 2026-08-16)
