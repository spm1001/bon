...
- The fresh-clone error no longer recommends a Dolt reconnect when a live `items.jsonl` sits in `.bon/` (bon-zageme). A board that merely lost its `.bon/prefix` marker now gets prefix-restoration advice with the prefix *derived from the item IDs* (`bon init --prefix <derived>`) — following the old advice (`--backend dolt`) would have flipped a live JSONL board and orphaned its data. The Dolt-reconnect message fires only when `items.jsonl` is absent, and no longer lists `.bon/backend` as a missing marker (absent backend means JSONL — normal, not evidence).
