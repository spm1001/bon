...
- **Opening/closing context:** gather the equivalent time, host, Git state, board, room/knowledge paths, handoff ledger and session identity with ordinary read-only calls. Do not execute the full Claude `open-context.sh` or `close-context.sh`, whose session/transcript and side-effect assumptions have not been qualified for Codex. Existing small CLI/survey helpers may be reused after checking their entrypoint and inputs. Missing helper output means derive its information directly, not omit its purpose.
