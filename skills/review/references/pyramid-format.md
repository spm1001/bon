...
**One work-item per line** — a tick is atomic, so a second work-item riding in the same line dies with it: on 2026-08-30 a line was ticked when its cited card closed and took an unnoticed rider down with it, which the operator then had to re-mint as its own line.
