...
**And the breadcrumb follows the draw-down, not only the `/open`** (bon-monevu; Sameer's call, 2026-09-01). `bon work <id>` rewrites it for the board the card lives on, in the CLI, every time a claim is taken or resumed. The case that decided it: a session launched at `~` that `/open`ed `~/.claude` because that was where the unticked handoff sat, then drew down a notes-board card and spent its evening in a bench directory — its statusline said `.claude` for the rest of its life. Orientation is where you read; the claim is where you commit, and the glance should show the commitment. Nothing for you to do here beyond the `bon work` you were going to run anyway; the paragraph exists so you are not surprised when the arrow moves at draw-down.
...
3. **`bon work <id>`** — initialize tactical steps from `--what`
   ...
   - Also re-declares the statusline orientation breadcrumb for this board (bon-monevu) — so if you drew the card down on a different board from the one you `/open`ed, the statusline arrow moves to follow the work.
